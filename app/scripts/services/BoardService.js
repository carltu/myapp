...
angular.module('myappApp').service('BoardService', ['$modal', 'BoardManipulator', function ($modal, BoardManipulator) {
  ...
  return {
    ...
    addNewCard: function (board, column) {
      ...
      modalInstance.result.then(function (cardDetails) {
        if (cardDetails) {
          BoardManipulator.addCardToColumn(board, cardDetails.column, cardDetails.title, cardDetails.details, cardDetails.id);
        }
      });
    },
    kanbanBoard: function (board) {
      ...
      angular.forEach(board.columns, function (column) {
        ...
        angular.forEach(column.cards, function (card) {
          BoardManipulator.addCardToColumn(kanbanBoard, column, card.title, card.details, card.id);
        });
      });
      return kanbanBoard;
    },
